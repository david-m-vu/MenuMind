...
const Onboarding = ({ userProfile, setUserProfile }) => {
  ...
  // allow opening onboarding at a specific step (from Profile edits)
  const location = useLocation();
  const initialStep = (location?.state && location.state.step) || 1;

  const handleSubmit = () => {
    setUserProfile({
      dietaryConditions: conditions.map((el) => el.label),
      dietaryRestrictions: restrictions.map((el) => el.label),
    });
    navigate("/home");
  };
  ...
  // initialize from userProfile if present
  const [conditions, setConditions] = useState(() => {
    return (userProfile?.dietaryConditions || []).map((c) => ({
      value: c.toLowerCase().replace(/[^a-z0-9]+/g, "-"),
      label: c,
    }));
  });
  const [restrictions, setRestrictions] = useState(() => {
    return (userProfile?.dietaryRestrictions || []).map((c) => ({
      value: c.toLowerCase().replace(/[^a-z0-9]+/g, "-"),
      label: c,
    }));
  });
  ...
  return (
    <div id="Landing">
      <div className="landingHero">
        {step == 1 ? (
          <Step1 conditions={conditions} setConditions={setConditions} />
        ) : (
          <Step2
